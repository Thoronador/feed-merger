...
int main(int argc, char ** argv)
{
  ...
  if (std::difftime(feed.pubDate(), expectedChannelPubDate) != 0)
  {
    const std::time_t parsed = feed.pubDate();
    std::cout << "Publication date of channel does not match the expected value!"
              << std::endl << "Expected: " << ctime(&expectedChannelPubDate) << std::endl
              << "Expected (integer):" << static_cast<long int>(expectedChannelPubDate) << std::endl
  ...
  const std::time_t expectedChannelLastBuildDate = std::mktime(&tempTM);
  if (std::difftime(feed.lastBuildDate(), expectedChannelLastBuildDate) != 0)
  {
    const std::time_t parsed = feed.lastBuildDate();
    std::cout << "Last build date of channel does not match the expected value!"
              << std::endl << "Expected: " << ctime(&expectedChannelLastBuildDate) << std::endl
              << "Expected (integer):" << static_cast<long int>(expectedChannelLastBuildDate) << std::endl
              << "Parsed: " << ctime(&parsed) << std::endl
              << "Parsed (integer):" << static_cast<long int>(parsed) << std::endl;
    return 1;
  }
  //docs
  if (feed.docs() != "http://blogs.law.harvard.edu/tech/rss")
  {
    std::cout << "Error: Unexpected value for feed documentation URL: "
              << feed.docs() << "!" << std::endl;
    return 1;
  }
  //generator
  if (feed.generator() != "Weblog Editor 2.0")
  {
  ...
  //editor
  if (feed.managingEditor() != "editor@example.com")
  {
    std::cout << "Error: Unexpected value for managing editor's e-mail: "
              << feed.managingEditor() << "!" << std::endl;
    return 1;
  }
  //webmaster
  if (feed.webMaster() != "webmaster@example.com")
  {
    std::cout << "Error: Unexpected value for webmaster's e-mail: "
